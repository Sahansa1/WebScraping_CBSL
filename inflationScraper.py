...
def render_inflation_page():
    ...
    #     st.dataframe(df.head(8) if df is not None else pd.DataFrame(),
    #                  use_container_width=True)

    # if _csv_is_bad(df):
    #     st.error(
    #         "Couldn’t parse numbers yet. Ensure internet access & Selenium/requests, then click Rescrape.")
    #     return
    ...
    pivot = d.pivot_table(index="year", columns="month_num",
                          values=ycol, aggfunc="mean").sort_index()
    fig_hm = px.imshow(pivot, aspect="auto", labels=dict(
        color="Y-o-Y (%)"), height=220)
    fig_hm.update_xaxes(tickmode="array", tickvals=list(
        range(1, 13)), ticktext=[m[:3] for m in MONTHS])
