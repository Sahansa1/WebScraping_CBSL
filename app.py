...
selected_page = st.sidebar.radio(
    "Select Page", ["Home", "Policy Rates", "Exchange Rates", "Inflation"])
...
# --- Exchange Rates Page ---
    ...
    with tab1:
        if curr:
            fig = px.line(
                plot_data, x="Date", y=f"TT Rates -Buying {curr}", title=f"{curr} Over Time")
            st.plotly_chart(fig, use_container_width=True)

    with tab2:
        if curr:
            fig = px.line(
                plot_data, x="Date", y=f"TT Rates -Selling {curr}", title=f"{curr} Over Time")
            st.plotly_chart(fig, use_container_width=True)
